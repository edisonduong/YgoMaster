...
namespace YgoMasterClient
{
    class ConsoleHelper
    {
        public unsafe static void ExecuteCommand(string consoleInput)
        {
        ...
        unsafe static void HandleCommand(string consoleInput)
        {
            ...
            switch (splitted[0].ToLower())
            {
                ...
                case "methods":
                    {
                        ...
                        {
                            Console.WriteLine("Usage: methods <ClassName|FullName> [Namespace]");
                            Console.WriteLine("Example: methods TitleViewController YgomGame.Title");
                            break;
                        }

                        string classQuery = splitted[1];
                        string namespaceFilter = splitted.Length > 2 ? splitted[2] : null;
                        ...
                        IEnumerable<IL2Class> matches = assembly.GetClasses().Where(x =>
                            (!string.IsNullOrEmpty(namespaceFilter)
                                ? string.Equals(x.Name, classQuery, StringComparison.OrdinalIgnoreCase) &&
                                  string.Equals(x.Namespace ?? string.Empty, namespaceFilter, StringComparison.OrdinalIgnoreCase)
                                : string.Equals(x.Name, classQuery, StringComparison.OrdinalIgnoreCase) ||
                                  string.Equals(x.FullNameEx, classQuery, StringComparison.OrdinalIgnoreCase)));

                        if (!matches.Any() && string.IsNullOrEmpty(namespaceFilter))
                        {
                            matches = assembly.GetClasses().Where(x =>
                                x.Name.IndexOf(classQuery, StringComparison.OrdinalIgnoreCase) >= 0 ||
                                x.FullNameEx.IndexOf(classQuery, StringComparison.OrdinalIgnoreCase) >= 0);
                        }

                        IL2Class[] classes = matches.OrderBy(x => x.FullNameEx).ToArray();
                        if (classes.Length == 0)
                        {
                            Console.WriteLine("No classes matched '" + classQuery + "'. Try: classes " + classQuery);
                            break;
                        }

                        StringBuilder sb = new StringBuilder();
                        foreach (IL2Class classInfo in classes)
                        {
                            sb.AppendLine("//==================================");
                            sb.AppendLine("// " + classInfo.FullNameEx);
                            sb.AppendLine("//==================================");
                            foreach (IL2Method method in classInfo.GetMethods().OrderBy(x => x.Name))
                            {
                        ...
                        string safeClassName = new string(classQuery.Where(ch => !Path.GetInvalidFileNameChars().Contains(ch)).ToArray());
                        if (string.IsNullOrWhiteSpace(safeClassName))
                        {
                            safeClassName = "Class";
                        }
                        string path = Path.Combine(Program.ClientDataDir, "ClassMethods-" + safeClassName + ".txt");
                        try
                        {
                            File.WriteAllText(path, sb.ToString());
                            Console.WriteLine("Wrote methods for " + classes.Length + " class(es) to: " + path);
                        }
                        catch (Exception ex)
                        {
                            Console.WriteLine("Failed to write methods dump: " + ex);
                        }
                    }
                    break;
                case "hierarchy":
                ...
                case "dumpassets":
                    {
                        string dump = AssetHelper.DumpTrackedAssets();
                        string path = Path.Combine(Program.ClientDataDir, "LoadedAssets.txt");
                        try
                        {
                            File.WriteAllText(path, dump);
                            Console.WriteLine(dump);
                            Console.WriteLine("Wrote tracked asset dump to: " + path);
                        }
                        catch (Exception ex)
                        {
                            Console.WriteLine("Failed to write tracked asset dump: " + ex);
                        }
                    }
                    break;
                ...
                case "updatediff":// Dumps referenced enums / functions (use this to diff and update enums / funcs)
                    {
                        ...
                        {
                            Console.WriteLine("Failed to generate updatediff: " + error);
                        }
                    }
                    break;
                ...
                case "cardswithart":// List all cards with art (requires setup of YgoMaster/Data/CardData/)
                    ...
                    break;
                case "dumphome":// Dumps the home UI hierarchy to disk (diagnostic/dev tool)
                    {
                        HomeViewTweaks.DumpHomeHierarchy();
                    }
                    break;
